...
namespace FullOpaqueVFX
{
    public class VFX_SpellManager : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (currentSpell != null && IsActivationPressed() && !isOnCooldown)
            {
                StartCoroutine(CastSpell());
            }
        }

        // Use the new Input System if available. Fall back to old Input safely.
        private bool IsActivationPressed()
        {
            ...
            try
            {
                ...
                {
                    // Try to map UnityEngine.KeyCode name to UnityEngine.InputSystem.Key
                    if (Enum.TryParse<Key>(currentSpell.activationKey.ToString(), out var key))
                    {
                    ...
                    // Additional common fallback for alpha keys (KeyCode.Alpha1..Alpha0 -> Key.Digit1..Digit0)
                    string kcName = currentSpell.activationKey.ToString();
                    if (kcName.StartsWith("Alpha", StringComparison.OrdinalIgnoreCase))
                    {
                        string digit = kcName.Substring(5);
                        if (Enum.TryParse<Key>("Digit" + digit, out var digitKey))
                        {
                            var keyControl = Keyboard.current[digitKey];
                            if (keyControl != null)
                                return keyControl.wasPressedThisFrame;
                        }
                    }
            ...
            {
                // ignore and try legacy input below
            }

            // Fallback to legacy Input (wrapped in try/catch to avoid InvalidOperationException)
            try
            {
                return Input.GetKeyDown(currentSpell.activationKey);
            }
            catch
            {
                return false;
